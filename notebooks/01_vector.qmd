...
Open the store with `obstore` (direct S3 byte ranges) and let `zarr-python` do the chunk-level reads, exactly as the tutorial does.
...
Map the Salzburg bbox to pixel indices through the affine transform on the group attrs, then index the array directly. `y` is descending (north to south), so `ymax` maps to the top of the slice. We stride by 4 (~40 m) to keep the array small for Colab.

```{python}
dx, _, x_origin, _, dy, y_origin = g.attrs["spatial:transform"]
x0, x1 = math.floor((xmin - x_origin) / dx), math.ceil((xmax - x_origin) / dx)
y0, y1 = math.floor((ymax - y_origin) / dy), math.ceil((ymin - y_origin) / dy)
STRIDE = 4
salzburg_emb = emb[-1, :, y0:y1:STRIDE, x0:x1:STRIDE]  # (band, y, x), most recent year
print(f"shape = {salzburg_emb.shape}, MB = {salzburg_emb.nbytes / 1e6:.2f}")
```
...
```{python}
arr = salzburg_emb.transpose(1, 2, 0).astype(np.float32)  # (y, x, band)
