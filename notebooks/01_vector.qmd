...
X = np.array([emb_at(lon, lat) for pts in TRAINING.values() for lon, lat in pts])
y = np.array([j for j, pts in enumerate(TRAINING.values()) for _ in pts])
clf = LogisticRegression(max_iter=1000).fit(X, y)
print(
    f"coef = {clf.coef_.shape}  bias = {clf.intercept_.shape}  "
    f"train acc = {clf.score(X, y):.0%}"
)
```
