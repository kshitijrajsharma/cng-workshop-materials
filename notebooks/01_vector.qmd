...
BBOX = tuple(json.loads((ROOT / "data" / "aoi" / "salzburg_bbox.json").read_text())["bbox"])
...
print("geo metadata:")
print(json.dumps(geo, indent=2)[:600])
```
...
def pmtiles_source(url_or_path):
    s = str(url_or_path)
    if s.startswith(("http://", "https://")):
        def get(offset, length):
            req = urllib.request.Request(
                s, headers={"Range": f"bytes={offset}-{offset + length - 1}"}
            )
            with urllib.request.urlopen(req) as resp:
                return resp.read()
        return get
    return MmapSource(open(s, "rb"))  # noqa: SIM115  source held by Reader

header = Reader(pmtiles_source(AUSTRIA_PMTILES)).header()
