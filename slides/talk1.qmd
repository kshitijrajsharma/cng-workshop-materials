...
:::
::: {.column width="50%"}
HTTP has supported partial downloads since 1999 (`Range: bytes=X-Y`). The file lives on object storage. The client fetches **only the byte ranges it needs**: 1 request for the header, N requests for the chunks touching the AOI. The rest of a 1.2 GB file is never transferred.
