...
subtitle: "Day 1 · CIVIS Salzburg 2026 · 40 min + 5 min Q&A"
...
::: {.column width="30%"}
<div class="speaker-photo-wrap"><img src="../assets/images/photo.png" alt="Kshitij Raj Sharma" data-lightbox="false"></div>
:::
...
CDE Student , AI Engineer at HOTOSM. 8+ years working with planet-scale OpenStreetMap, Overture Maps, and large-scale aerial imagery. Experience with developing on & with multiple STAC catalogs

Author of [VirtuGhan](https://github.com/virtughan/virtughan) and other several open-source geospatial tools, focused on GeoAI at the intersection of OSM, Earth Observation, and cloud-native data formats.
...
::: {.fragment}
The Sentinel satellites alone produce **~20 TB of raw data every day**. After processing, ESA disseminates **~250 TB of Sentinel products per day** for services to society. The Copernicus Data Space Ecosystem's published archive has now crossed **78 PB**, growing by terabytes every hour.
:::

::: {.fragment}
That is one programme. Add commercial constellations, drone fleets, OpenStreetMap, lidar, and every model that produces its own derived layers, and "download the file, then crop" stops being a sensible default.
:::

::: footer
Sources: <https://guide.cloudnativegeo.org/> · [ESA Belém 2025](https://eo4society.esa.int/wp-content/uploads/2025/02/3_CDSE_Belem.pdf) · [CDSE Annual Report 2024](https://dataspace.copernicus.eu/news/2025-12-4-copernicus-data-space-ecosystem-cdse-releases-annual-report-2024)
:::
...
::: {.callout-warning}
**Storing data in the cloud does not, on its own, solve geospatial's data problems.** The file format itself has to be designed for partial reads, otherwise you are back to downloading whole scenes from a more expensive disk.
:::
...
::: incremental
1. Find the source for your AOI + time window
2. Read only the chunks you need
...
Describing the data itself is important for discovery, but also for efficient access. A cloud-optimized format is designed so that:

::: incremental
- Metadata or a spec provides addresses for the data blocks.
- Metadata sits in a known location, in a consistent format.
- All metadata loads in a single read.
- Libraries use that metadata to fetch subsets via addressable chunks, internal tiling, or both.
:::

::: {.fragment}
Parallel reads, partial reads, no subsetting service in the middle. The format itself does the work, and a catalog can describe it. That catalog is **STAC**.
:::

::: footer
Source: [Cloud-Optimized Geospatial Formats Guide, "The Opportunity"](https://guide.cloudnativegeo.org/)
:::
...
## Tomorrow, we write the code {.smaller}
...
[Built with [Quarto + RevealJS](https://quarto.org/docs/presentations/revealjs/)]{.tiny}
