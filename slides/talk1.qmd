  ...
  revealjs:
    ...
    include-in-header:
      text: |
        ...
        <style>.bi { vertical-align: -0.12em; }</style>
---
...
<iframe src="https://radiantearth.github.io/stac-browser/#/external/earth-search.aws.element84.com/v1" width="1100" height="540" style="display: block; margin: 0 auto; border: 1px solid #ccc;"></iframe>

## OpenAerialMap / HOTOSM

<iframe src="https://radiantearth.github.io/stac-browser/#/external/api.imagery.hotosm.org/stac/" width="1100" height="540" style="display: block; margin: 0 auto; border: 1px solid #ccc;"></iframe>

## STAC for ML models

<iframe src="https://radiantearth.github.io/stac-browser/#/external/stac.fair.krschap.tech" width="1100" height="540" style="display: block; margin: 0 auto; border: 1px solid #ccc;"></iframe>

## EOPF Sample Service

<iframe src="https://radiantearth.github.io/stac-browser/#/external/stac.core.eopf.eodc.eu" width="1100" height="540" style="display: block; margin: 0 auto; border: 1px solid #ccc;"></iframe>
...
:::
::: {.column width="50%"}
The file lives on object storage. The client fetches **only the byte ranges it needs**: 1 request for the header, N requests for the chunks touching the AOI. The rest of a 1.2 GB file is never transferred.
...
::: {.callout-important}
**No tile server.** A static file on object storage + a viewer in the browser is the entire stack.
:::
:::
:::
...
<iframe src="https://pmtiles.io/?url=https://huggingface.co/datasets/kshitijrajsharma/cng-workshop-materials/resolve/main/austria_buildings.pmtiles" width="1100" height="500" style="display: block; margin: 0 auto; border: 1px solid #ccc;"></iframe>

::: footer
~1.5GB of buildings served via HTTP range requests from Hugging Face. No tile server.
:::
...
- Embeddings: it is recommended to use **blob storage with HTTP range-read support**. Pick **Zarr** for regular grids, **GeoParquet** for sparse or irregular.
...
::: incremental
- Query **Overture Maps** (~200 GB of planet-scale GeoParquet on S3) with **DuckDB**, extract POIs for Salzburg
- Inspect **GeoParquet 1.1** metadata on a 1.4 GB Austria buildings file on Hugging Face, bbox-stream the Salzburg subset via the `bbox` covering column
- Inspect **FlatGeobuf** header + packed R-tree on the same file, bbox-stream over HTTP
- Render the result on a **lonboard** map (GeoArrow → deck.gl)
- Read the **PMTiles** header via HTTP range requests, view the whole Austria in `pmtiles.io`
:::
...
<i class="bi bi-globe-americas"></i> धन्यवाद · Thank you · Danke · Bonjour
...
<i class="bi bi-envelope"></i> krschap@proton.me &nbsp; · &nbsp; <i class="bi bi-person"></i> Kshitij Raj Sharma
